    ...
    <>
      <div className="text-left p-4 rounded-md border-2 w-full">
        ...
        <div className="flex flex-row items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="icon" onClick={toggleReading}>
              {isReading ? (
                  ...
                  <DropdownMenuPortal>
                    <DropdownMenuSubContent>
                      <DropdownMenuItem
                        onClick={() =>
                          window.open(
                            `https://www.google.com/search?q=${format(answer)}`
                          )
                        }
                      >
                        Google
      ...
      <div className="flex flex-row justify-evenly w-full mt-2 gap-2 sticky bottom-0">
        <Input
          ...
          className={"w-full " + (showing ? "block" : "hidden")}
        />
        <div className="flex flex-row gap-2">
          <Button
            className="justify-items-end"
            onClick={() => {
              if (isReading && !showing) {
                toggleReading();
                setShowing(!showing);
              } else if (showing) {
                setShowing(!showing);
          ...
          <Button variant="secondary" className="justify-items-end">
            Next
          </Button>
        </div>
      </div>
